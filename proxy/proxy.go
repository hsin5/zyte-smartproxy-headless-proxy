...
func makeProxyLayers(conf *config.Config, crawleraExecutor executor.Executor, statsContainer *stats.Stats) []layers.Layer {
	...
	if len(conf.DirectAccessHostPathRegexps) > 0 && len(conf.NotDirectAccessHostPathRegexps) > 0 {
		proxyLayers = append(proxyLayers, customs.NewDirectAccessLayer(conf.DirectAccessHostPathRegexps, conf.NotDirectAccessHostPathRegexps))
		...
		proxyLayers = append(proxyLayers, customs.NewDirectAccessLayer(nil, conf.NotDirectAccessHostPathRegexps))
	}
