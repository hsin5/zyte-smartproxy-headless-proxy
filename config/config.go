...
type Config struct {
	...
	NotDirectAccessHostPathRegexps []string `toml:"not_direct_access_hostpath_regexps"`
...
// MaybeSetNotDirectAccessHostPathRegexps sets a list of regular
// expressions for direct access.
func (c *Config) MaybeSetNotDirectAccessHostPathRegexps(value []string) {
	if len(value) > 0 {
		c.NotDirectAccessHostPathRegexps = value
	}
}
